...
export function AcousticIsleMain() {
  ...
  const handleRoyaltyUpdate = (amount: string, stemId: string) => {
    const numericAmount = parseFloat(amount.replace('$', ''));
    setTotalRoyalty((prev) => prev + numericAmount);
    
    setActiveStems((prev) => {
      const exists = prev.find(s => s.id === stemId);
      if (exists) return prev;
      return [...prev, { id: stemId, name: stemId.replace(/_/g, ' '), amount }];
    });
  };
        ...
        <nav className="flex-1 space-y-6">
          <section>
            ...
            <p className="text-sm leading-relaxed text-foreground/80">
              An interactive bridge connecting global producers to the unrecorded heritage of the Andaman Islands.
            </p>
          </section>

          <section className="space-y-4">
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Principles</h2>
            <div className="flex items-center gap-3 text-sm text-foreground/70">
              <ShieldCheck className="w-4 h-4 text-accent" />
              <span>Ethical AI Sourcing</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-foreground/70">
        ...
        {!isJamming ? (
          <div className="max-w-xl text-center space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            <div className="inline-block p-1 px-3 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-semibold tracking-wider uppercase mb-2">
              AI-Native Musical Sandbox
            </div>
            <h2 className="text-4xl md:text-6xl font-headline font-bold leading-tight">
              Jam with <span className="text-accent">Protected</span> Heritage.
            </h2>
            <p className="text-lg text-muted-foreground leading-relaxed">
              Connect your camera and microphone. Let the AI Ethnomusicologist analyze your energy and provide culturally accurate accompaniments.
            </p>
            <Button 
