      ...
      <div className="space-y-4">
        ...
        >
          ...
          <div className="mt-2 flex items-center gap-2 px-3 py-1 rounded-full bg-accent/10 border border-accent/20 relative z-10">
            ...
            <p className="text-[9px] font-bold text-accent uppercase tracking-widest">Verified Ledger</p>
          </div>
        </motion.div>
      </div>
            ...
            <AnimatePresence initial={false}>
              {(events || []).length === 0 && localStems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-48 text-center space-y-3">
                  ...
                  <p className="text-[10px] text-muted-foreground italic uppercase tracking-widest">Establishing secure link...</p>
                </div>
              ) : (
                (events || []).map((event) => (
                  <motion.div 
